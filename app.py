...
def apply_space_ui():
    st.markdown("""
        ...
        .desktop-icon {
            ...
            border: 1px solid rgba(0, 242, 255, 0.4); border-radius: 30px;
            height: 250px; display: flex; flex-direction: column;
            justify-content: flex-end; align-items: center;
            transition: 0.5s ease; box-shadow: 0 20px 50px rgba(0,0,0,0.8);
            position: relative; overflow: hidden;
        }
        .desktop-icon:hover { transform: scale(1.05); border-color: #00f2ff; box-shadow: 0 0 60px rgba(0, 242, 255, 0.5); }
        .icon-label {
        ...
        .window-pane {
            background: rgba(0, 0, 0, 0.9) !important; backdrop-filter: blur(30px);
    ...
    with win_col:
        ...
        if st.session_state.active_window == "SYNTHESIS":
            ...
            with c1:
                ...
                if st.button("GENERA BILD", key="gen_img"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                ap = st.text_area("MUSIK-BESKRIVNING")
                dur = st.slider("Längd (sekunder)", 5, 30, 15)
                if st.button("GENERA MUSIK", key="gen_audio"):
                    with st.status("KOMPONERAR..."):
                        audio_res = replicate.run("meta/musicgen:671ac645cf51591bc3754da5ff3f4523d44373308dfc9496883cd2050119e8de", input={"prompt": ap, "duration": dur})
            ...
            st.info("Video-modul redo för implementering.")
