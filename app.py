...
# --- 3. INITIALISERING (FIX FÖR ATTRIBUTEERROR) ---
if "page" not in st.session_state:
    ...
    st.session_state.update({
        ...
        "last_img": None,
        "enhance": False, # DEFINIERAD HÄR FÖR ATT UNDVIKA KRASCH
        "lang": "SV"
    })
...
if st.session_state.page == "SYNTH":
    ...
    st.markdown(f"<h2 style='color:{accent};'>🪄 HYPER-SYNTH</h2>", unsafe_allow_html=True)
    
    c1, c2, c3 = st.columns([0.5, 0.25, 0.25])
    user_p = c1.text_input("VISION PROMPT:", placeholder="Beskriv bilden...")
    aspect = c2.selectbox("FORMAT:", ["1:1", "16:9", "9:16"], index=0)
    # Fix: Använd session_state direkt i toggle
        ...
        if user_p:
            with st.status("Neural process...") as status:
                ...
                if st.session_state.enhance:
                    prompt_instr = f"Expand this to a detailed cinematic 8k image prompt in English: {user_p}"
                    trans = safe_replicate_run("LLM", {"prompt": prompt_instr, "max_new_tokens": 150})
    ...
    for i, item in enumerate(reversed(st.session_state.library)):
        with cols[i % 3]:
            st.image(item['url'])
            if st.button(f"Sätt bakgrund", key=f"wall_{i}"):
