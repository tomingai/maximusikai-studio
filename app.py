    ...
    with t[3]: # ARKIV
        ...
        for p in reversed(my):
            with st.expander(f"📁 {p['name'].upper()}"):
                st.image(str(p["url"]))
                c1, c2 = st.columns(2)
                if c1.button(L["set_bg"], key=f"s_{p['id']}"): st.session_state.app_bg = p["url"]; st.rerun()
                try: c2.download_button(L["download"], requests.get(p["url"]).content, f"{p['name']}.png", "image/png", key=f"d_{p['id']}")
                except: pass
