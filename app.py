...
# --- 3. INITIALISERING (Lagt till brightness) ---
...
# --- 4. UI ENGINE (Nu med Lumina-kontroll) ---
accent = st.session_state.accent
# Beräknar mörkhets-overlay baserat på 1-10
darkness = (11 - st.session_state.brightness) / 10

st.markdown(f"""
    <style>
    [data-testid="stAppViewContainer"]::before {{
        content: ""; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
        background: rgba(0, 0, 0, {darkness}); z-index: -1; transition: 0.3s;
    }}
    ...
    .stTextInput>div>div>input, .stSelectbox>div>div {{
        ...
        color: white !important; border: 1px solid {accent}44 !important; border-radius: 10px !important;
    }}
    .stButton>button {{ 
        border: 1px solid {accent}88 !important; color: white !important; 
        background: {accent}22 !important; border-radius: 12px; height: 3.5rem; font-weight: bold; width: 100%;
    }}
...
c_nav, c_lum = st.columns([0.75, 0.25])
...
with c_lum:
    st.session_state.brightness = st.slider("🔅 LUMINA", 1, 10, st.session_state.brightness)

st.markdown('</div>', unsafe_allow_html=True)

# --- 6. MODULER (Helt oförändrade) ---
if st.session_state.page == "SYNTH":
    ...
    if st.button("🚀 STARTA NEURAL KEDJA"):
        if user_p:
            ...
            raw_expanded = "".join(list(replicate.run("meta/meta-llama-3-8b-instruct", input={"prompt": f"Expand to a {st.session_state.style} 8k image prompt: {user_p}. Respond only with prompt."})))
        ...
        for i, item in enumerate(reversed(st.session_state.library)):
            with grid[i % 3]:
                ...
                if st.button("VÄLJ", key=f"ark_{i}"):
                    st.session_state.last_img = item['url']; st.session_state.wallpaper = item['url']; st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
