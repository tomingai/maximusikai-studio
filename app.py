...
# --- 2. HJÄLPFUNKTIONER FÖR NEDLADDNING ---
    ...
    with win_col:
        ...
        if st.session_state.active_window == "SYNTHESIS":
            ...
            if st.session_state.synth_res:
                ...
                st.download_button(label="📥 LADDA NER BILD", data=get_media_data(st.session_state.synth_res), file_name="maximus_art.png", mime="image/png", use_container_width=True)

        # --- MODULE: AUDIO ---
        elif st.session_state.active_window == "AUDIO":
            ap = st.text_input("BESKRIV MUSIK:")
            if st.button("KOMPONERA LJUD", use_container_width=True):
            ...
            if st.session_state.audio_res:
                ...
                st.download_button(label="📥 LADDA NER LJUD", data=get_media_data(st.session_state.audio_res), file_name="maximus_audio.mp4", mime="audio/mp4", use_container_width=True)

        # --- MODULE: VIDEO ---
        elif st.session_state.active_window == "VIDEO":
            vp = st.text_input("BESKRIV SCEN:")
            if st.button("RENDERA VIDEO", use_container_width=True):
                with st.status("RENDERAR..."):
                    res = replicate.run("lucataco/luma-dream-machine", input={"prompt": vp})
                    st.session_state.video_res = res
                st.rerun()
            if st.session_state.video_res:
                st.video(st.session_state.video_res)
                st.download_button(label="📥 LADDA NER VIDEO", data=get_media_data(st.session_state.video_res), file_name="maximus_video.mp4", mime="video/mp4", use_container_width=True)
        ...
        # --- MODULE: BG ENGINE ---
            ...
            st.write("### 🎞️ GALLERI")
            cols = st.columns(4)
            for idx, img_url in enumerate(reversed(st.session_state.bg_gallery[-4:])):
                with cols[idx]:
