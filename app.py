...
with st.sidebar:
    ...
    st.caption("v3.5.0 // TOMAS INGVARSSON")
...
if "REPLICATE_API_TOKEN" in st.secrets:
    ...
    tab_list = L["t"] if show_admin else L["t"][:-1]
    ...
    with tabs[3]: # ARKIV
        if st.session_state.is_pro and st.button("EXPORT ZIP"):
            buf = io.BytesIO(); zf = zipfile.ZipFile(buf, "w")
            zf.writestr("history.json", json.dumps(st.session_state.gallery)); zf.close()
    ...
    if show_admin:
        with tabs[5]: # ADMIN
            st.subheader("🛠️ TOMAS ANALYTICS")
            col1, col2 = st.columns(2)
            stats = load_data(STATS_FILE)
            with col1:
                st.metric("Totala produktioner", len(st.session_state.gallery))
                if stats:
                    df = pd.DataFrame(stats)
                    st.write("**Populära stilar:**")
                    st.bar_chart(df['mode'].value_counts())
            with col2:
                st.write("**Senaste Feedback:**")
                feedbacks = load_data(FEEDBACK_FILE)
                for f in reversed(feedbacks):
                    st.info(f"{f['time']} | {f['user']}: {f['msg']}")
                if st.button("Rensa Feedback"): save_data([], FEEDBACK_FILE); st.rerun()
