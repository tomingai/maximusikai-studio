...
def handle_replicate_output(output):
    """Säkerställer att vi får ren Bytes-data oavsett om det är en URL eller ström."""
    if not output: return None, None
    
    try:
        # Hantera generatorer/listor (Flux-standard)
        ...
        # Om det är ett File-objekt från Replicate
        if hasattr(output, 'read'):
            return output.read(), None
            
    except Exception as e:
        st.error(f"Data-extrahering misslyckades: {e}")
    return None, None
...
if st.session_state.page == "SYNTH":
    ...
    if st.session_state.last_img: 
        # FIX: Vi använder BytesIO för att säkerställa att Streamlit kan läsa datan korrekt
...
# MODUL: ARKIV
    ...
    for i, item in enumerate(reversed(st.session_state.library)):
        grid[i % 4].image(io.BytesIO(item['data']))
    st.markdown('</div>', unsafe_allow_html=True)
