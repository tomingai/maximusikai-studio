...
# --- 2. DESIGN (EXTRA LJUSA FLIKAR & NEON) ---
st.markdown("""
    <style>
    .stApp {
    ...
    /* KRITVITA OCH EXTRA TYDLIGA FLIKAR */
...
if "REPLICATE_API_TOKEN" in st.secrets:
    ...
    st.error("Lägg till din REPLICATE_API_TOKEN i Secrets!")
    api_ready = False
...
if api_ready:
    ...
    with tab1:
        ...
        with c1:
            proj_name = st.text_input("Namnge ditt projekt:", f"PROJ-{len(st.session_state.gallery)+1}")
            m_ide = st.text_area("Beskriv din vision:", "En futuristisk stad i lila neon-regn")
            if st.button("🚀 STARTA PRODUKTION"):
                with st.status("🏗️ MAXIMUSIKAI BYGGER...") as status:
                    try:
                        ...
                        m_url = get_url(replicate.run("facebookresearch/musicgen", input={"prompt": "Cyberpunk music, melodic", "duration": 8}))

                        # Spara till Session (Bibliotek)
                        entry = {
                            "name": proj_name,
                            "video": v_url,
                            "lyrics": lyrics,
    ...
    with tab5:
        ...
        if not st.session_state.community_feed:
            st.info("Inga delade projekt än.")
        else:
