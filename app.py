...
# VERSION: 9.3.1 | STATUS: STABIL | REGLER 1-17
st.set_page_config(page_title="MAXIMUSIK AI OS v9.3.1", layout="wide", initial_sidebar_state="collapsed")
...
if st.session_state.page == "DESKTOP":
    ...
    with nc[7]:
        status_label = "⚠️" if st.session_state.system_alert else "🟢"
        st.markdown(f'<div style="text-align:center; padding-top:5px;"><span class="status-dot"></span><br><small style="color:white; font-size:0.5rem;">{status_label}</small></div>', unsafe_allow_html=True)
    ...
    if st.session_state.page == "SYNTH":
        ...
        if st.button("🔥 STARTA RENDER", use_container_width=True):
            prog = st.progress(0, text="Ansluter...")
            img_raw = safe_replicate_run("black-forest-labs/flux-schnell", {"prompt": p}, "SYNTH_RENDER")
            if img_raw:
                url = str(img_raw[0]) if isinstance(img_raw, list) else str(img_raw)
        ...
        for i, item in enumerate(reversed(st.session_state.library)):
            ...
            except: st.error("Kunde inte hämta fil.")
