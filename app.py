...
st.set_page_config(page_title="MAXIMUSIK AI OS v7.1 FINAL", layout="wide", initial_sidebar_state="collapsed")
    ...
    with st.spinner("Visualizing Soundwaves..."):
        try:
            ...
            res = replicate.run("black-forest-labs/flux-schnell", 
                               input={"prompt": f"Cinematic digital art inspired by: {descr}"})
            url = get_url(res)
...
def apply_ui():
    ...
    st.markdown(f"""
        ...
        .window-box {{ background: rgba(0, 5, 12, 0.9); backdrop-filter: blur(25px); border: 1px solid {accent}44; border-radius: 30px; padding: 35px; margin-top: 20px; }}
...
if st.session_state.page == "DESKTOP":
    ...
    st.markdown(f"<p style='text-align:center; color:{st.session_state.accent_color}; opacity:0.7;'>SYSTEM OS v7.1 // ALL MODULES ACTIVE</p>", unsafe_allow_html=True)
    cols = st.columns(5)
    ...
    with win_col:
        ...
        if st.session_state.page == "SYNTH":
            ...
            if st.session_state.last_image_res:
                ...
                if st.button("🎵 SONIFY"): st.session_state.last_audio_res = sonify_image(st.session_state.last_image_res); st.rerun()

        elif st.session_state.page == "AUDIO":
            up_audio = st.file_uploader("LADDA UPP:", type=["mp3", "wav"])
            if up_audio:
                st.audio(up_audio)
            ...
            if not f_lib: st.write("TOMT")
                ...
                for idx, item in enumerate(reversed(f_lib)):
                    with l_cols[idx % 3]:
                        ...
                        if c1.button("🗑", key=f"del_{idx}"): st.session_state.library.remove(item); st.rerun()
                        if item['type'] == "image" and c2.button("🖼", key=f"bg_{idx}"): st.session_state.wallpaper = item['url']; st.rerun()
                        if item['type'] == "image" and c3.button("🎵", key=f"s_{idx}"): st.session_state.last_audio_res = sonify_image(item['url']); st.rerun()
                        if item['type'] == "audio" and c3.button("🖼", key=f"v_{idx}"): st.session_state.last_image_res = visualize_audio(item['url']); st.rerun()
                        try:
                            resp = requests.get(item['url'])
                            c4.download_button("💾", resp.content, f"file_{idx}.{'png' if item['type']=='image' else 'wav'}", key=f"dl_{idx}")
                        except: pass
                        st.markdown("---")
